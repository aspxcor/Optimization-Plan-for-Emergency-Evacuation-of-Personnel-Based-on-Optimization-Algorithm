...
class genetic2ChromWithoutCVProblem(ea.Problem):  # 继承Problem父类
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        for i in range(SInTurns.shape[0]):
            ...
            SInTurnOfCar1 = [SInTurn[:tmpBlock1[0] + 1]] + [SInTurn[tmpBlock1[_] + 1:tmpBlock1[_ + 1] + 1] for _ in range(0,self.B-2)] + [SInTurn[tmpBlock1[self.B-2] + 1:]]
            TInTurnOfCar1 = [TInTurn[:tmpBlock1[0] + 1]] + [TInTurn[tmpBlock1[_] + 1:tmpBlock1[_ + 1] + 1] for _ in range(0,self.B-2)] + [TInTurn[tmpBlock1[self.B-2] + 1:]]
